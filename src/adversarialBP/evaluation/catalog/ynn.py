...
class YNN():
    """
    Computes y-Nearest-Neighbours for generated adversarial_examples
    ...
    def __init__(self, dataset_manager, mlmodel, one_hot_matrix, hyperparameters):
    ...
    def predict_label(self, neighbours):
        ...
        for i in range(0,neighbours.shape[0]):
            ...
            prediction = self.mlmodel.predict_proba(sequence_tensor, 'inference')
            prediction = prediction.round()[0].item()
            predicted_labels.append(prediction)

        return predicted_labels

    def _ynn(self, adversarial_examples, num):
        count_ynns = []

        for i in range(0,len(adversarial_examples)):
            adversarial_example = adversarial_examples[i]
            adversarial_example = adversarial_example[np.newaxis,:,:]
            neighbours = self.find_closest_sequences(adversarial_example, self.factuals, num)
            ...
            for i, row in adversarial_examples.iterrows():
                if np.any(row.isna()):
                    raise ValueError(f"row {i} did not contain a valid adversarial_example")
    ...
    def get_evaluation(self, factuals, adversarial_examples):
        ...
        ynn = self._ynn(adversarial_examples, self.nn)
